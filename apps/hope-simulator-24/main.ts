type Phase = "calm" | "uneasy" | "betrayal" | "collapse" | "descent" | "gift";
type Mood = "good" | "bad" | "soft";
...
const hopeEl = document.getElementById("hopeValue") as HTMLElement;
...
const upgradesEl = document.getElementById("upgrades") as HTMLElement;
const hopeBtn = document.getElementById("hopeBtn") as HTMLButtonElement;
const advanceBtn = document.getElementById("advanceBtn") as HTMLButtonElement | null;
const scrambleBtn = document.getElementById("scrambleBtn") as HTMLButtonElement;
const beamEl = document.getElementById("beam") as HTMLElement;
const flareEl = document.getElementById("flare") as HTMLElement;
const portholeLabel = document.getElementById("portholeLabel") as HTMLElement;
const podModel = document.getElementById("podModel") as HTMLElement;
const pieceEls: Record<keyof State["visuals"], HTMLElement> = {
  antenna: document.getElementById("piece-antenna") as HTMLElement,
  amplifier: document.getElementById("piece-amplifier") as HTMLElement,
  coil: document.getElementById("piece-coil") as HTMLElement,
  mirror: document.getElementById("piece-mirror") as HTMLElement,
};
...
const upgrades: Upgrade[] = [
  {
    ...
    name: "歪んだアンテナ",
    desc:
      "ポッド外殻から掘り出して再接続。<br>希望獲得量をわずかに増やす。",
    price: 24,
    visual: "antenna",
    apply: () => (state.click += 1),
  },
...
const phaseOfDay = (day: number): Phase => {
  if (day <= 3) return "calm";
  if (day <= 7) return "uneasy";
  if (day <= 12) return "betrayal";
  if (day <= 17) return "collapse";
  if (day <= 23) return "descent";
  return "gift";
};

function formatHope(value: number): string {
  if (!Number.isFinite(value)) return "HOPE = ∞";
...
function log(message: string, mood: Mood = "soft") {
...
function renderDashboard() {
  ...
  Object.entries(state.visuals).forEach(([id, on]) => {
    const el = pieceEls[id as keyof State["visuals"]];
    if (!el) return;
...
function render() {
  ...
  const phases: Phase[] = ["calm", "uneasy", "betrayal", "collapse", "descent", "gift"];
...
function collectHope() {
  ...
  if (phase === "uneasy") gain += (Math.random() - 0.5) * 1.5;
  if (phase === "collapse") gain -= Math.random() * 2;
  if (phase === "descent" && Math.random() < 0.25) gain *= -1;
...
function buyUpgrade(upgrade: Upgrade) {
  if (state.finale || upgrade.bought) return;
  ...
  upgrade.bought = true;
  upgrade.price *= 1.6;
  if (upgrade.visual) state.visuals[upgrade.visual] = true;
  log(`${upgrade.name} をポッド内プリンタで生成・接続。`, "good");
  renderUpgrades();
  render();
}

function advanceDay(source: "auto" | "manual" = "manual") {
  if (state.finale) return;
  if (state.day >= 24) return;
  state.day += 1;
  state.hope += 5 + state.restBonus;
  log(
    source === "auto"
      ? `Day ${state.day} に自動で移行。計器が勝手に刻んでいる。`
      : `Day ${state.day} に移行。ポッドがきしんでいる。`,
    "soft"
  );
  if (state.day === 4) log("UIにちらつき。気のせいか？", "bad");
  if (state.day === 8) log("購入すると希望が落ちる噂。", "bad");
  if (state.day === 13) log("数値が負にもなる。危険。", "bad");
  if (state.day === 18) log("画面が揺れているのは船体か心か。", "bad");
  if (state.day === 24) {
    triggerFinale();
    return;
  }
...
function scramble() {
  ...
  if (phase === "descent") document.body.classList.add("shake");
...
function tick() {
  ...
  if (phase === "uneasy") delta += (Math.random() - 0.5) * 2;
  if (phase === "betrayal") delta += (Math.random() - 0.5) * 4;
  if (phase === "collapse") delta += (Math.random() - 0.5) * 6;
  if (phase === "descent") delta += (Math.random() - 0.5) * 12;
  state.hope += delta;
